...
const GenerateBtn = () => {//add in Home page

  const {user,setShowLogin}=useContext(AppContext)//get user and setShowLogin from context api
    const navigate=useNavigate()

    const onClickHandler=()=>{
        if(user){//same as button in header component
            navigate("/result")
        }else{
            setShowLogin(true)
        }
